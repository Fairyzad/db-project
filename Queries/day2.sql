...
--Count()you can specify the column name so count(Column name)
...
---FIND OUT THE Min  SALARY IN EMPLOYEES TABLE
--EXCLUDE THE SALARY 2100 NOT EQUAL SIGN!= OR <>
SELECT MIN(SALARY)
FROM EMPLOYEES
WHERE SALARY <> 2100;
...
--DISPLAY THE MAX,AVG ,SUM SALARY OF EACH DEPARTMENT
...
--DISPLAY THE MIN SALARY OF EACH JOB_ID
SELECT JOB_ID,
       MIN(SALARY) AS "LOWEST SALARY"
FROM EMPLOYEES
GROUP BY JOB_ID;
...
-- IF YOU ARE USING AGGREGATE FUNCTIONS(MEANS MAX,SUM,MIN,AVG) IN CONDITION AFTER GROUP BY
